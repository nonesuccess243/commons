...
public class DbHelper
{
        ...
        public static void CREATE_TABLE( String tableName, String sql ) throws SQLException
        {
                ...
                if( resultSet.next())
                {
                        log.error("表{}已经存在" + resultSet.getString("TABLE_CAT") + "\t"  
                                        + resultSet.getString("TABLE_SCHEM") + "\t"  
                                        + resultSet.getString("TABLE_NAME") + "\t"  
                                        + resultSet.getString("TABLE_TYPE"), tableName);  
                }else
                {
                        log.debug("使用以下sql建表：\n{}", sql);
